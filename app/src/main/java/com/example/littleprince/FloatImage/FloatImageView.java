...
public class FloatImageView extends FrameLayout implements IFloatImageView {
    ...
    private TextView tv_info;//删
    private RelativeLayout videoViewWrap;
    private RelativeLayout content_wrap;
    private ImageView imageView;
    private ImageView iv_zoom_btn;//拖动大小按钮图

    private WindowManager windowManager = null;
    private WindowManager.LayoutParams windowParams = null;
    private FloatImageParams floatImageParams = null;
    ...
    public FloatImageView(Context context, FloatImageParams floatImageParams, WindowManager.LayoutParams wmParams) {
        super(context);
        Log.d("context4",String.valueOf(context.hashCode()));
        this.floatImageParams = floatImageParams;
        this.windowParams = wmParams;
        init();
    }
    ...
    private void initData() {
        Log.d("initData","khlkjgfkjgkghgkjh");
        context = getContext().getApplicationContext();
        Log.d("context5",String.valueOf(context.hashCode()));
        Log.d("initData,null?",String.valueOf(context==null));
        windowManager = FloatImageUtil.getWindowManager(context);
        statusBarHeight = floatImageParams.statusBarHeight;
        screenWidth = floatImageParams.screenWidth;
        screenHeight = floatImageParams.screenHeight - statusBarHeight;//要去掉状态栏高度
        videoViewMargin = floatImageParams.videoViewMargin;
        mMaxWidth = floatImageParams.mMaxWidth;
        mMinWidth = floatImageParams.mMinWidth;
        mRatio = floatImageParams.mRatio;
        //起点
        startX = floatImageParams.x;
        startY = floatImageParams.y;
        //isSdkGt23 = Build.VERSION.SDK_INT >= 23;
        // >=23的部分手机缩放会卡顿，系统弹窗更新位置迟缓不够平滑
    }

    private void initView() {
        Log.d("initView","khlkjgfkjgkghgkjh");
        LayoutInflater inflater = LayoutInflater.from(getContext().getApplicationContext());
        View floatView = inflater.inflate(R.layout.float_image, null);
        ...
        imageView=floatView.findViewById(R.id.iv_live_cover);
        imageView.setImageURI(Uri.parse(floatImageParams.imagePath));
        Uri uri=Uri.parse("file://"+floatImageParams.imagePath);
        try {
            Bitmap bp= MediaStore.Images.Media.getBitmap(context.getContentResolver(),uri);

            Log.d("","&&&&&&&&&&&&&&&&"+String.valueOf(bp.getHeight())+"&&&&&&&&&&&&&&&&&&&&");
            Log.d("","&&&&&&&&&&&&&&&&"+String.valueOf(bp.getWidth())+"&&&&&&&&&&&&&&&&&&&&");
        ...
        int lastViewWidth = floatImageParams.contentWidth;
        ...
        private void handleMoveEvent(MotionEvent event) {
            ...
            if (distance >= 2) {//控制刷新频率
                ...
                if (!isSdkGt23) {
                    ...
                    if (windowParams.width != mMaxWidth) {
    ...
    private void updateFloatWindowSize(int width) {
        int currentWidth = windowParams.width;
    ...
    private final Runnable updateWindowPostionRunnable = new Runnable() {
        @Override
        public void run() {
            updateWindowXYPosition(windowParams.x + dx, windowParams.y + dy);
        }
    };
    ...
    private void fixWindowXYPostion() {
        ...
        if (windowParams.x + width >= screenWidth) {
            windowParams.x = screenWidth - width - 1;// 不让贴近右边和底部
        }
        if (windowParams.x <= 0) {
            windowParams.x = 0;
        }
        int height = content_wrap.getHeight();
        if (windowParams.y + height >= screenHeight) {
            windowParams.y = screenHeight - height - 1;
        }
        if (windowParams.y <= statusBarHeight) {
    ...
    private void handleZoomStatus() {
        //左，上贴边时，隐藏dragView
        boolean isLeft = windowParams.x <= 0;
        boolean isTop = windowParams.y <= statusBarHeight;
        if (isLeft || isTop) {
            displayZoomView();
            // 贴边时设置视频margin
            if (isLeft && isTop) {
    ...
    private void showZoomView() {
        if (!isEdit ) {//&& isSdkGt23 只有6.0及以上才显示缩放按钮
            ...
            videoViewWrap.setBackgroundColor(getResources().getColor(R.color.float_image_border_edit));
            isEdit = true;
        }
    }
    ...
    private void displayZoomView() {
        ...
        videoViewWrap.setBackgroundColor(getResources().getColor(R.color.float_image_border_normal));
    }
    ...
    @Override
    public FloatImageParams getParams() {
        floatImageParams.contentWidth = getContentViewWidth();
        floatImageParams.x = windowParams.x;
        floatImageParams.y = windowParams.y;
        floatImageParams.width = windowParams.width;
        floatImageParams.height = windowParams.height;
